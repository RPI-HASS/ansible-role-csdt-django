...
`csdt_django_sync_script_cron_enabled`\
(default: `false`)\
Set this to `true` to add a cron entry for copying the database.

`csdt_django_sync_script_remote`\
(default: `prod.fqdn.edu`)\
Set this to the server FQDN that the script will remotely log into for the database and media copy.
